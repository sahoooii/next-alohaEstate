...
import { EditPropertyType } from '@/utils/types';

const EditPropertyClient = ({ property }: { property: EditPropertyType }) => {
	const { _id } = property;
	const propertyId = _id.toString();

	const defaultAmenities: Amenity[] = JSON.parse(property.amenities[0]);

	return (
		<>
			<BreadCrumps name={property.name} link='/rentals' title='My Properties' />
